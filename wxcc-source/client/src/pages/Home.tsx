...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Header */}
      <div className="h-14 bg-slate-950 border-b border-white/8 flex items-center px-6 md:px-10 justify-between">
        <div className="flex items-center gap-3">
          <div className="w-7 h-7 bg-cyan-400/15 rounded-lg flex items-center justify-center border border-cyan-400/30">
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg, #0d1535 0%, #0A0E27 50%, #0a1a1f 100%)" }}
      >
        <div
          className="absolute inset-0 pointer-events-none"
          style={{ background: "radial-gradient(ellipse 60% 100% at 0% 50%, rgba(0,217,255,0.07) 0%, transparent 60%)" }}
        />
        <div
              ...
              <div className="flex gap-1.5 items-center">
                {IMPACT_STATS.map((_, i) => (
                  <span
                    ...
                    style={{ width: i === statIndex ? "16px" : "5px", height: "5px", background: i === statIndex ? "rgba(0,217,255,0.8)" : "rgba(255,255,255,0.2)" }}
                  />
                ))}
              </div>
            </div>
      ...
      <div className="border-b border-white/8" style={{ background: "linear-gradient(180deg, #060918 0%, #080C20 100%)" }}>
            ...
            {JOURNEY_STAGES.map((stage, i) => {
              ...
              return (
                <div key={stage.id} className="flex flex-col md:flex-row items-stretch flex-1">
                  {/* Card */}
                  <div className="flex-1 rounded-xl border border-white/8 bg-white/[0.025] p-5 flex flex-col gap-3 hover:border-cyan-400/20 hover:bg-white/[0.04] transition-all duration-300">
                    {/* Top row: chapter + icon */}
                    <div className="flex items-center justify-between">
                      <span
                        className="font-mono text-xs font-bold px-1.5 py-0.5 rounded border text-cyan-400/70 border-cyan-400/20 bg-cyan-400/8"
                      >
                        {stage.chapter}
                      </span>
                      <div className="w-7 h-7 rounded-lg flex items-center justify-center border border-cyan-400/15 bg-cyan-400/8">
                    ...
                    <div className="mt-auto flex items-center gap-1.5 pt-3 border-t border-white/6">
                      <Phone className="w-3 h-3 text-cyan-400/40" />
                      <span className="text-xs text-cyan-400/40 font-mono">SMS to patient</span>
                    </div>
                  </div>
                  ...
                  {i < JOURNEY_STAGES.length - 1 && (
                    <div className="hidden md:flex items-center flex-shrink-0 px-2">
                      <div className="flex items-center gap-0.5">
                        <div
                          className="w-8 h-px"
                          style={{ background: "repeating-linear-gradient(to right, rgba(0,217,255,0.35) 0px, rgba(0,217,255,0.35) 4px, transparent 4px, transparent 8px)" }}
                        />
                        <svg width="6" height="9" viewBox="0 0 6 9" fill="none" className="flex-shrink-0" style={{ color: "rgba(0,217,255,0.4)" }}>
          ...
          <div className="lg:col-span-2 lg:sticky lg:top-6 lg:self-start">
            <Card className="border border-white/10 bg-card shadow-xl overflow-hidden">
              ...
              <div className="relative flex justify-center items-center py-10 px-4">
                <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse 80% 70% at 50% 50%, rgba(0,217,255,0.05) 0%, transparent 70%)" }} />
                <div className={`absolute inset-0 pointer-events-none transition-opacity duration-700 ${phonePulse ? "opacity-100" : "opacity-0"}`} style={{ background: "radial-gradient(ellipse 90% 80% at 50% 50%, rgba(0,217,255,0.18) 0%, transparent 65%)" }} />
                <div
                  ...
                  style={{
                    ...
                    boxShadow: phonePulse
                      ? "0 30px 70px rgba(0,0,0,0.7), 0 0 80px rgba(0,217,255,0.28), 0 0 0 3px rgba(0,217,255,0.15)"
                        ...
                        <div className="space-y-2.5">
                          <div className="flex items-center gap-2 mb-2.5">
                            <div className="w-7 h-7 rounded-full bg-slate-300 flex items-center justify-center flex-shrink-0">
                              <span className="text-slate-600 font-bold" style={{ fontSize: "9px" }}>HS</span>
                            </div>
                        ...
                        <div className="space-y-3">
                          <div className="flex items-center gap-2 mb-3">
                            <div className="w-7 h-7 rounded-full bg-cyan-500 flex items-center justify-center flex-shrink-0">
                              <span className="text-white font-bold" style={{ fontSize: "9px" }}>HS</span>
                            </div>
                            <span className="text-slate-500 text-xs">Health System · now</span>
                          </div>
                          <div className="bg-slate-100 rounded-2xl rounded-tl-sm px-3 py-2.5">
                            <p className="text-slate-800 leading-snug text-xs">{activePhoneStage.phoneMessage}</p>
                          </div>
                          <div className="bg-cyan-500 rounded-2xl px-3 py-2">
                    ...
                    <div key={stage.id} className={`flex gap-3 transition-all duration-500 ${isLocked ? "opacity-25" : "opacity-100"}`}>
                      <div className="flex-shrink-0 flex items-start pt-[14px] z-10">
                        <div className={`w-[10px] h-[10px] rounded-full border-2 transition-all duration-500 ${
                          isTriggered ? "border-green-400 bg-green-400 shadow-[0_0_8px_rgba(74,222,128,0.6)]" :
                          isNext ? "border-cyan-400 bg-cyan-400/20 shadow-[0_0_8px_rgba(0,217,255,0.4)]" :
                      ...
                      <div
                        className={`flex-1 rounded-xl border transition-all duration-500 ${
                          isTriggered ? "border-green-400/35 bg-green-950/20" :
                          isNext ? "border-cyan-400/40 bg-card" :
                          "border-white/6 bg-card/50"
                        }`}
                        style={isNext ? { boxShadow: "0 0 0 1px rgba(0,217,255,0.15), 0 8px 32px rgba(0,217,255,0.06)" } : undefined}
                      >
                        ...
                        <div className="flex items-center gap-2 px-4 pt-3 pb-3">
                          <span className={`font-mono text-xs font-bold px-1.5 py-0.5 rounded border transition-all duration-500 flex-shrink-0 ${
                            isTriggered ? "text-green-400 border-green-400/30 bg-green-500/10" :
                            isNext ? "text-cyan-400 border-cyan-400/30 bg-cyan-400/8" :
                            "text-white/20 border-white/8"
                          }`}>
                            {stage.chapter}
                          </span>
                          <h3 className={`font-bold transition-colors duration-500 flex-1 ${
                            isTriggered ? "text-green-400 text-sm" :
                        ...
                        <div className="px-4 pb-3">
                          {isTriggered ? (
                            <div className="flex items-center justify-center gap-2 py-2 bg-green-500/10 border border-green-400/25 rounded-lg">
                              <span className="text-sm font-semibold text-green-400">Sent to patient</span>
                            </div>
                          ) : isLocked ? (
                            <button
                            ...
                            <Button
                              ...
                              className="w-full bg-cyan-500 hover:bg-cyan-400 active:bg-cyan-600 text-slate-950 font-bold text-sm h-10 border-0 shadow-[0_4px_20px_rgba(0,217,255,0.3)]"
                            >
                        ...
                        {isExpanded && (
                          <div className="px-4 pb-4 pt-3 border-t border-white/6 space-y-4">
                            ...
                            <div className="rounded-lg overflow-hidden border border-white/8">
                              <img
                                src={`https://placehold.co/600x280/0d1535/334155?text=Workflow+Diagram+—+${encodeURIComponent(stage.label)}`}
                            ...
                            {isTriggered && revealedSteps > 0 && (
                              <div className="flex gap-2 flex-wrap">
                                {FLOW_STEPS.slice(0, revealedSteps).map((step) => (
                                  <div key={step} className="flex items-center gap-1.5 px-2 py-1 bg-green-500/8 border border-green-400/15 rounded-md">
                                    <span className="w-1 h-1 rounded-full bg-green-400 flex-shrink-0" />
                                    <span className="text-xs text-green-400/60 font-mono">{step}</span>
                                  </div>
                                ))}
                              </div>
                            )}
      ...
      {showCompleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
          ...
          {/* Modal */}
          <div
            className="relative z-10 w-full max-w-lg rounded-3xl border-2 border-green-400/50 overflow-hidden shadow-2xl"
            style={{
              background: "linear-gradient(160deg, #0d2214 0%, #0A0E27 60%)",
              boxShadow: "0 0 0 1px rgba(74,222,128,0.2), 0 40px 80px rgba(0,0,0,0.8), 0 0 120px rgba(74,222,128,0.08)",
            }}
          >
            {/* Top accent line */}
            <div className="h-1 bg-gradient-to-r from-green-500 via-cyan-400 to-green-500" />

            <div className="px-10 py-10 text-center">
              {/* Icon */}
              <div
                className="inline-flex items-center justify-center w-20 h-20 rounded-full border-2 border-green-400 mb-6"
                style={{ background: "radial-gradient(circle, rgba(74,222,128,0.15) 0%, transparent 70%)", boxShadow: "0 0 40px rgba(74,222,128,0.2)" }}
              >
                <Check className="w-10 h-10 text-green-400" />
              </div>

              <h2 className="text-4xl font-black text-white mb-2">Journey Complete</h2>
              <p className="text-green-400 font-semibold mb-5">Zero phone calls. 100% digital.</p>
              ...
              <button
                onClick={resetJourney}
                className="inline-flex items-center gap-2 px-7 py-3 border border-green-400/40 hover:border-green-400 text-green-400 hover:bg-green-400/10 rounded-xl font-semibold text-sm transition-colors mb-8"
              >
                ↺ Run Another Demo
              </button>
              ...
              <div className="pt-6 border-t border-white/8 flex flex-col items-center gap-3">
                ...
                <div className="p-2 rounded-xl border border-white/10 bg-slate-950/40">
                  <img
                    src="https://api.qrserver.com/v1/create-qr-code/?size=128x128&data=https://thesenate.io&color=00D9FF&bgcolor=0A0E27&margin=8&qzone=1"
                    alt="Scan to book a workshop"
                    className="w-28 h-28 rounded-lg"
                  />
                </div>
                <p className="text-xs text-cyan-400/50 font-mono tracking-wide">thesenate.io</p>
              </div>
            </div>
          </div>
        </div>
